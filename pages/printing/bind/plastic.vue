...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "Переплёт на пластиковую пружину",
    },
  ],
});

// Конфигурация полей + серверные данные
const pageKey: PageConfigKey = "bind-plastic";
const { fields, productionDays, pageImage, pageDescription, pageExamples } =
  useServerOrderConfig({
    pageKey,
    defaultImage: "/img/bind/2.png",
    defaultDescription: "",
    defaultShowMacketButton: false,
...
const submitOrder = async () => {
  await submitOrderFn({
    productType: "Переплёт на пластиковую пружину",
    printType: "Переплёт на пластиковую пружину",
    fields,
