...
const { fields, productionDays, pageDescription, pageExamples } =
  useServerOrderConfig({
    pageKey,
    defaultImage: "/img/repli/1.png",
    defaultDescription: "",
...
const selectBookBtn = (index: number) => {
  ...
  const foldingFieldItem = fields.find((f: OrderField) => f.label === "Сложение");
...
watch(
  () =>
    paperDensityFieldComputed.value?.type === "dropdown"
      ...
      : null,
  (newValue: { label: string; price: number } | null) => {
    if (newValue && newValue.label && creasingFieldComputed.value?.type === "toggle") {
...
// Инициализация кнопок буклета после загрузки полей
const initBookButtons = () => {
...
// Инициализируем кнопки после загрузки полей с сервера
watch(
  () => fields.length,
  () => {
    if (fields.length > 0) {
